...
class LogDAO:
    ...
    async def export_logs_to_csv(
        ...
        """
        Export all logs to a CSV at `file_path`, including only `fields`.

        Args:
            file_path: local path where the CSV will be written.
            fields:    list of field names to include (must match your log schema).
            batch_size: how many docs to fetch at once (tweak for memory vs speed).
        """
        # Build a projection so Mongo only returns those fields
        projection = {f: 1 for f in fields}
        # We'll page through the collection in batches
        cursor = (
            log_collection
            .find({}, projection)
        ...
        with open(file_path, "w", newline="", encoding="utf-8") as fp:
            ...
            async for doc in cursor:
                # Ensure any ObjectId or missing key is stringified or blank
